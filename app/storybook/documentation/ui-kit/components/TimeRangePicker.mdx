...
allows users to pick a predefined time range, a custom fixed date range, or a dynamic period up to the current date,
with additional settings to fine-tune the selection such as last `n`-days. Features include disabling specific options
and a fully controllable state via props.
