...
If you'd prefer to handle fetching on your own, you can pass the `accessToken` prop to the `AccessTokenProvider` and an `onAccessTokenExpired` callback to handle token expiration. The provided access token will be served to all of its child components:
